-- Kategori isimleri benzersiz olmalı
ALTER TABLE categories 
ADD CONSTRAINT uq_categories_name UNIQUE (category_name);

-- Bölge isimleri benzersiz olmalı
ALTER TABLE regions 
ADD CONSTRAINT uq_regions_name UNIQUE (region_name);

-- Aynı bölgede aynı şehir ismi bir kez tanımlanabilsin
ALTER TABLE cities 
ADD CONSTRAINT uq_cities_name_region UNIQUE (city_name, region_id);

-- Tedarikçi firma adı benzersiz
ALTER TABLE suppliers 
ADD CONSTRAINT uq_suppliers_company_name UNIQUE (company_name);

-- Nakliyeci firma adı benzersiz
ALTER TABLE shippers 
ADD CONSTRAINT uq_shippers_company_name UNIQUE (company_name);

-- Ürün isimleri benzersiz
ALTER TABLE products 
ADD CONSTRAINT uq_products_name UNIQUE (product_name);

-- Aynı sipariş için aynı tarihli ödeme sadece bir kez girilebilsin
ALTER TABLE payments 
ADD CONSTRAINT uq_payments_order_date UNIQUE (order_id, payment_date);

-- Aynı sipariş için aynı ürün yalnızca bir kez iade edilsin
ALTER TABLE returns 
ADD CONSTRAINT uq_returns_order_product UNIQUE (order_id, product_id);
